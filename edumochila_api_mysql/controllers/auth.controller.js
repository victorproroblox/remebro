...
export async function login(req, res) {
  try {
    ...
    return res.json({
      ...
      access_token: token,
      token_type: "Bearer",
      usuario: { id_us: usuario.id_us, nom_us: usuario.nom_us, tip_us: usuario.tip_us },
    });
  } catch (e) {
...
export async function register(req, res) {
  try {
    ...
    const id_estado = req.body.id_estado !== undefined && req.body.id_estado !== null
    ...
    if (tip === 3 && !cedula_us) {
      return res
        .status(422)
        .json({ estatus: "error", mensaje: "cedula_us es obligatoria para maestros (tip_us=3)." });
    }

    if (cp_us && !/^\d{5}$/.test(cp_us)) {
      return res.status(422).json({ estatus: "error", mensaje: "cp_us debe ser 5 dígitos." });
    }

    if (id_estado !== null && !Number.isInteger(id_estado)) {
      return res.status(422).json({ estatus: "error", mensaje: "id_estado debe ser entero." });
    }

    // Unicidades (suaves; la BD también lo valida)
    if (nom_us) {
      const dupUser = await Usuario.findOne({ where: { nom_us } });
      if (dupUser) {
        return res.status(409).json({ estatus: "error", mensaje: "nom_us ya está en uso." });
      }
    }
    if (email_us) {
      const dupEmail = await Usuario.findOne({ where: { email_us } });
      if (dupEmail) {
        return res.status(409).json({ estatus: "error", mensaje: "email_us ya está en uso." });
      }
    }
    ...
    if (id_estado !== null) {
      ...
      if (!estadoOk) {
        return res.status(422).json({ estatus: "error", mensaje: "id_estado no existe." });
      }
    }
    ...
    const nuevo = await Usuario.create({
      ...
      email_us, nom_us, pass_us: hash, tip_us: tip,
      cp_us, mun_us, id_estado, colonia_us, calle_us, ni_us, ne_us,
    ...
    return res.status(201).json({
      ...
      usuario: { id_us: nuevo.id_us, nom_us: nuevo.nom_us, tip_us: nuevo.tip_us },
    });
  } catch (e) {
    // Duplicado por índice único (fallback)
    if (e?.original?.code === "ER_DUP_ENTRY" || e?.original?.errno === 1062) {
      return res.status(409).json({ estatus: "error", mensaje: "Usuario o email ya existe." });
    }
    return res
      .status(500)
...
export async function logout(_req, res) {
  // Con JWT, el "logout" es borrar el token en el cliente.
  return res.json({
    estatus: "exitoso",
    mensaje: "Sesión cerrada correctamente (borra el token en el cliente).",
  });
}
